       ...
       Serial.println(httpResponseCode);
        if(httpResponseCode == 200) {
          ...
          if(payload.indexOf("open") > 0){
            ...
            m = scale.read_average(10);
            int b_height = getDistanceFromUltrasonic();
            for(int i =0; i <= 4; i++) {
              delay(2000);
              float k =  scale.read_average(10);
              Serial.println(abs(k-m));//get load sensor value
              if(abs(k-m) > 150 ) {
                // weight was added
                weightAdded = true;
                break;
            ...
            height = getDistanceFromUltrasonic(); // get the height using ultrasonic sensor
            Serial.println(abs(height-b_height));
            if(abs(height-b_height) > 5 ) {
              delay(100);
        ...
        if(weightAdded) {
            ...
            String postServerPath = serverName + "/unit_value_history/" + uid + "/";
              ...
              String httpRequestData="{\"weight_value\":"+ String(m/1000,3)+", \"height_value\": "+height+", \"redeemed\": false}";
