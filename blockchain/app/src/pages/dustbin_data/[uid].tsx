...
const dustbin_data: NextPage = (props) => {
  let { data, error } = useSWR(
    `https://solanabin.pythonanywhere.com/dustbin_data/${
      useRouter().query.uid
    }/`,
    fetcher
  );
  ...
  const handleModalOpen = async () => {
    ...
    try {
      let c = await axios.get(
        `https://solanabin.pythonanywhere.com/open_dustbin/${uid}/`
      );
    } catch (e) {
  ...
  const clearTheInterval = async (e, newPayloadFound, payloadNotFound) => {
    ...
    try {
      let c = await axios.get(
        `https://solanabin.pythonanywhere.com/close_dustbin/${uid}/`
      );
    } catch (e) {
      console.log("unable to close dustbin");
    }
  ...
  async function listenForChange() {
    ...
    let e = setInterval(async () => {
      let d = await axios.get(
        `https://solanabin.pythonanywhere.com/listen_for_changes/${uid}/`
      );
      setDataForPaayment(d.data.data);
      let newDataLessThan60Seconds = d.data.data.recent < 60;
      let timeGreaterThan60Seconds =
        (((new Date() as any) - time) as any as number) > 60000;
            ...
            <div className="group relative">
              <div className="grid grid-cols-4 gap-2 relative w-full h-80 bg-white  z-0 rounded-lg ">
                ...
                <div className="col-span-2">
                  <div className="mt-12">
                    <h1 className="text-base font-semibold text-gray-900  ml-1">{`Bin-id : ${uid.substring(
                      0,
                      4
                    )}`}</h1>
                ...
                <div className=" flex flex-col h-12 justify-start mx-auto">
                  <div className=" bg-green-700 rounded-full text-white p-3">
                    <h2>{`${
                      (parseInt(data.weight_filled) / parseInt(data.capacity)) *
                      100
                    }%`}</h2>
                  </div>
                </div>
              ...
              {!placedPayload && (
                <div className="max-w-2xl mx-auto my-0 py-5 sm:py-24 lg:py-32 lg:max-w-none">
                  <div className=" space-y-12 lg:space-y-0 lg:grid lg:grid-cols-3 lg:gap-x-6">
                    <div className="group relative">
                      <div
                        className="grid place-items-center relative w-full h-12 bg-darkGreen rounded-lg overflow-hidden group-hover:opacity-75 sm:aspect-w-2 sm:aspect-h-1 sm:h-64 lg:aspect-w-1 lg:aspect-h-1"
  ...
  function anotherData() {
    return <div>you have placed payload</div>;
  }
    ...
    <div className="max-w-7xl h-screen mx-auto px-4 sm:px-6 lg:px-8">
      {showTimeout && (
        <div className="bg-blue-500 text-white rounded-md">
          <h3 className="pl-5">Timout Open again</h3>
        </div>
      )}
      {!placedPayload && data && getData()}
